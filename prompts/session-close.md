...
5. Rewrite `activity/CURRENT.md` with only: the activity-record path, current state in two or three sentences, the single next action, and anything that should not be retried and why. Information whose only purpose is to identify current execution state or the next work item belongs in `CURRENT.md`, not in `architecture/`, `AGENTS.md`, or `FAMILIAR.md`.
